...
parser.add_argument('--model-path',
                    ...
                    # default='./runs/wrn_svhn_best.pth.tar',
                    default='./runs/wrn_mnist_best.pth.tar',
...
parser.add_argument('--attack-method', default='PGD', help='attack method')
...
if args.dataset == 'cifar10':
    ...
    testset = torchvision.datasets.SVHN(root='../data', train='test', download=True, transform=transform_test)
elif args.dataset == 'mnist':
    testset = torchvision.datasets.MNIST(root='../data', train='test', download=True, transform=transform_test)
...
def _pgd_whitebox(filter,
    ...
    filted_data = filter(X_pgd)
    err_pgd = (model(filted_data).data.max(1)[1] != y.data).float().sum()
    if(args.attack_method =='BIM'):
        print('err bim (white-box): ', err_pgd)
    else:
        print('err pgd (white-box): ', err_pgd)
    return err, err_pgd


def _cw_whitebox(filter,
                 model,
                 X,
                 y,
                 epsilon=args.epsilon,
...
def _fgsm_whitebox(filter,
    ...
    filted_data = filter(X_fgsm)
    err_pgd = (model(filted_data).data.max(1)[1] != y.data).float().sum()
    print('err fgsm (white-box): ', err_pgd)
    return err, err_pgd


def _mim_whitebox(filter,
                  model,
                  X,
                  y,
                  epsilon=args.epsilon,
...
def _df_whitebox(filter, model, X, y):
    out = model(filter(X))
    err = (out.data.max(1)[1] != y.data).float().sum()
    X_df = Variable(X.data, requires_grad=True)
    attacker = DeepFool(10, overshoot=args.epsilon, max_iter=args.num_steps)
    X_df = attacker.attack(model, X_df)
    filted_data = filter(X_df)
    err_df = (model(filted_data).data.max(1)[1] != y.data).float().sum()
    print('err deepfool (white-box): ', err_df)
    return err, err_df
...
def eval_adv_test_whitebox(filter, model, device, test_loader, attack_method):
    """
    ...
    for data, target in test_loader:
        ...
        if attack_method == 'PGD':
            err_natural, err_robust = _pgd_whitebox(filter, model, X, y)
        elif attack_method == 'CW':
            err_natural, err_robust = _cw_whitebox(filter, model, X, y)
        elif attack_method == 'MIM':
            err_natural, err_robust = _mim_whitebox(filter, model, X, y)
        elif attack_method == 'FGSM':
            err_natural, err_robust = _fgsm_whitebox(filter, model, X, y)
        elif attack_method == 'BIM':
            err_natural, err_robust = _pgd_whitebox(filter, model, X, y, random=False)
        elif attack_method == 'DF':
            err_natural, err_robust = _df_whitebox(filter, model, X, y)
        else:
            raise NotImplementedError
    ...
    print('natural_acc: ', 1-natural_err_total/total)
    print('robust_acc: ', 1-robust_err_total/total)
...
def main():
    ...
    if args.white_box_attack:
        ...
        filter = FilterNet(1, 0.25, net_mode_train=False).to(device)
        filter_ckpt = torch.load('./runs/fnet_basic/fnet_type2_best.pth.tar')
        filter.load_state_dict(filter_ckpt['state_dict'])
        
        model = WideResNet(depth=34, widen_factor=10, num_classes=200 if args.dataset == 'timagenet' else 10).to(device)
        # model = resnet56().to(device)
        # args.model_path = 'runs/resnet_svhn_best.pth.tar'
        # model = vgg16().to(device)
        # args.model_path = 'runs/vgg_svhn_best.pth.tar'
        
        checkpoint = torch.load(args.model_path)
        model.load_state_dict(checkpoint['state_dict'])

        eval_adv_test_whitebox(filter, model, device, test_loader, args.attack_method)
    else:
        # black-box attack
